...
However, these methods are not without their flaws. It is possible for a handful of floating-point operations to yield a completely incorrect result. And in use cases such as computer-assisted proofs, we can often trade a little performance for certainty in our answers.
...
\begin{proof}
    ...
    ($\Leftarrow$) Let $x$ be a Cauchy sequence. Define $y_m \mapsto x_{N(1/2^m)}$ where $N(\varepsilon)$ is the index where all subsequent points differ by less than $\varepsilon$. Then $y$ is a \FCCS.
\end{proof}

\section{Arithmetic and closure properties}

With Cauchy sequences, we define arithmetic operations simply by working pointwise on each element. But with \FCCS, we need to do extra work to preserve the fast convergence property. For example, when two approximations are added together, their errors are added as well; this means that the error of the result is \textit{twice} that of the inputs. So the addition routine must shift the inputs one step to the right, halving the error to compensate.
...
\begin{Proposition}[Reciprocal]
    Let $x$ be a \FCCS\ which is not zero. Then $\left(x^{-1}\right)_k \mapsto \left(x_{k+2c}\right)^{-1}$ is a \FCCS, where $c$ is defined as in Lemma~\ref{apartness}.
\end{Proposition}
...
\begin{Corollary}[Division]
    $(x/y)_k \mapsto \left(x \cdot y^{-1}\right)_k$ is a \FCCS.
\end{Corollary}
