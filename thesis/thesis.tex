...
\begin{proof}
    ...
    $\abs{x_n} > 1/2^n \Rightarrow \abs{x_n} = 1/2^n + \epsilon$
    for some $\epsilon \in \Q$.

    Therefore
    \[
        \frac{1}{2^n} > \frac{1}{2^n} + \epsilon - \abs{x_{n+k}}
        \quad \Rightarrow \quad
        \abs{x_{n+k}} > \epsilon \ .
    \]

    Let $c \geq n$ be the least natural number such that $1/2^c \leq \epsilon$. Then
    \[ \abs{x_{c+k}} > \epsilon \geq 1/2^c \ . \qedhere \]
\end{proof}
